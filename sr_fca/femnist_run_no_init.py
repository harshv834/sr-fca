...
config["results_dir"] = os.path.join(
    config["results_dir"], config["dataset"] + "_no_init", "seed_{}_copy".format(seed)
)
...
config["total_clients"], _, train_data, test_data = read_data(
...
config["selected_clients"] = random.sample(
    config["total_clients"], config["num_clients"]
)
